const STEPS: { title: string; body: string }[] = [
  ...
  },
  {
    ...
    body: 'the disk is backed up before anything is written, so putting it back is one click. eject and wait for the restart — do not pull the cable. if a mic ever refuses to start, hold the white + grey buttons during startup to get the disk back and fix the file.',
  },
]
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-ink/40 p-4 sm:p-10">
      <div role="dialog" aria-modal="true" aria-label="how to use mic gnome"
        ...
        <p className="label mt-3 leading-relaxed">
          the EP&ndash;2350 fx-mic is configured by dropping a <code>config.json</code> and up to four wav files onto
          it over usb-c. break that json and the mic will not start. mic gnome exists so you never write that file
          by hand.
        </p>
