...
describe('guide progress, read off the bench', () => {
  it('starts on step 1 with nothing done and step 4 marked optional', () => {
    expect(stepStatuses(fresh, off)).toEqual(['current', 'todo', 'todo', 'optional', 'todo', 'todo'])
  })
  ...
  it('loading a library pack ticks the first three steps at once', () => {
    ...
    expect(stepStatuses(LIBRARY[0].config, off)).toEqual(['done', 'done', 'done', 'optional', 'current', 'todo'])
  })
  ...
  it('the last two steps come from the virtual write and the download', () => {
    expect(stepStatuses(LIBRARY[0].config, { written: true, downloaded: false }).slice(4)).toEqual(['done', 'current'])
